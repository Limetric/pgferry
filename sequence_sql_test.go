...
func TestResetSequenceStatements_QuotesSchemaAndSequenceRegclass(t *testing.T) {
	...
	stmts := resetSequenceStatements("order", table, col, sequenceResetWithDDL)
	if len(stmts) != 3 {
...
func TestResetSequenceStatements_QuotesNonTrivialSequenceName(t *testing.T) {
	...
	stmts := resetSequenceStatements("app", table, col, sequenceResetWithDDL)
...
func TestResetSequenceStatements_DataOnlySkipsSequenceDDL(t *testing.T) {
	table := Table{PGName: "events"}
	col := Column{PGName: "id", Extra: "auto_increment"}

	stmts := resetSequenceStatements("app", table, col, sequenceResetDataOnly)
	if len(stmts) != 1 {
		t.Fatalf("statement count = %d, want 1: %#v", len(stmts), stmts)
	}
	if !strings.Contains(stmts[0], `SELECT setval('"app"."events_id_seq"'::regclass`) {
		t.Fatalf("setval statement = %q", stmts[0])
	}
	if strings.Contains(stmts[0], "CREATE SEQUENCE") || strings.Contains(stmts[0], "SET DEFAULT") {
		t.Fatalf("data_only statement should not contain sequence DDL: %q", stmts[0])
	}
}
