...
func (s *sqliteSourceDB) IntrospectSchema(db *sql.DB, _ string) (*Schema, error) {
	...
	for i := range tables {
		...
		// If no PK from indexes, build from PRAGMA table_info pk column
		if t.PrimaryKey == nil {
			pk := buildSQLitePKFromColumns(cols)
			if pk != nil {
				t.PrimaryKey = pk
			}
		}
...
func introspectSQLiteColumns(db *sql.DB, tableName string) ([]Column, map[string]bool, error) {
	...
	for rows.Next() {
		...
		// Parse precision/scale from declared type like NUMERIC(10,2) or VARCHAR(255)
	...
	for _, col := range cols {
		if strings.EqualFold(col.ColumnType, "integer") {
			// Check if this is the sole PK column
			isPK := false
			pkCount := 0
			for _, c := range cols {
				if isSQLitePKColumn(db, tableName, c.SourceName) {
					pkCount++
					if c.SourceName == col.SourceName {
						isPK = true
					}
				}
			}
			if isPK && pkCount == 1 {
				autoIncrCols[col.SourceName] = true
			}
		}
	}
...
func isSQLitePKColumn(db *sql.DB, tableName, colName string) bool {
	rows, err := db.Query(fmt.Sprintf("PRAGMA table_info(\"%s\")", strings.ReplaceAll(tableName, "\"", "\"\"")))
	if err != nil {
		return false
	}
...
func buildSQLitePKFromColumns(cols []Column) *Index {
	// Build PK from columns that have pk > 0 in PRAGMA table_info
	// We stored OrdinalPos but need pk info separately.
	// Actually we need to re-query. But we can store a helper...
	// For now, the introspection already builds PKs from indexes.
	// This function builds PKs from columns that are marked in PRAGMA table_info.
	return nil
}
...
func buildPKFromTableInfo(db *sql.DB, tableName string) (*Index, error) {
	...
	for i := 0; i < len(pkCols)-1; i++ {
		for j := i + 1; j < len(pkCols); j++ {
			if pkCols[j].pkPos < pkCols[i].pkPos {
