...
- MSSQL sources use `go-mssqldb` (pure Go, native TDS); introspection via `sys.tables`, `sys.columns`, `sys.types`, `sys.indexes`, `sys.foreign_keys` filtered by `sourceSchema`; external tables (`is_external`) are skipped; `nvarchar`/`nchar` `max_length` divided by 2 (UCS-2 bytes → chars); user-defined types resolved to base system types; default expressions have outer parentheses stripped; `NEXT VALUE FOR` sequence defaults omitted with semantic warnings; `IDENTITY` columns mapped to `auto_increment` convention; computed columns detected via `is_computed`; `timestamp`/`rowversion` mapped to `bytea` (not datetime); `datetime2`/`time` scale mapped to PG `timestamp(n)`/`time(n)` (clamp 6); `uniqueidentifier` byte reordering for mixed-endian UUIDs; `money`/`smallmoney` → `numeric` to avoid float precision loss (non-finite floats error during copy); non–B-tree indexes (columnstore, hash, XML, spatial) skipped with warnings; `sql_variant` read via `TRY_CAST` to `nvarchar(max)`
