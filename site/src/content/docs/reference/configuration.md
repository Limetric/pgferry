...
| `data_only` | bool | `false` | Load data into an existing schema, then advance existing sequences with `setval`. Sequences are resolved from the target catalog — `pg_get_serial_sequence` first, then the column's `DEFAULT nextval(...)` expression, then pgferry's `{table}_{col}_seq` name — and the run fails if an auto-increment column has none. Requires the target role to disable and re-enable triggers on the selected target tables during the load. |
