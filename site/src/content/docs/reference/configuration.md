...
| `truncate_before_copy_schemas` | array of strings | `[schema]` | Target PostgreSQL schemas used only when `truncate_before_copy = "once"`. Set this to every schema in the multi-schema batch, and enable `"once"` only for the first pgferry config in that batch. |
