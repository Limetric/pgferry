# SQLite Hooks Example

Demonstrates all four hook phases using a SQLite source and PostgreSQL target.

## Hook phases

Hooks are SQL files executed at specific points during migration. All
occurrences of `{{schema}}` are replaced with the configured schema name at
runtime.
...
- The hook SQL runs against PostgreSQL, so the source database type does not change hook syntax.
